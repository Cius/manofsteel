...
public class PegawaiDetailCtrl_DataPokok_PengangkatanPNS extends GFCBaseCtrl implements Serializable {

	/**
	 * 
	 */
	private static final long serialVersionUID = -4005265307103199688L;
	private static final Logger logger = Logger.getLogger(PegawaiDetailCtrl_DataPokok_PengangkatanPNS.class);
	
	protected Window windowPegawaiDetail_DataPokok_PengangkatanPNS;
	private Borderlayout borderLayout_PegawaiDataPokok_PengangkatanPNS;
	...
	private Datebox dateBox_tmtpns;
	private Listbox comboBox_sumpah;
		
	private final String btnController_classPrefix = "btn_Pegawai_DataPokok_PengangkatanPNS_";
	private ButtonStatusCtrl buttonCtrl_Pegawai_DataPokok_PengangkatanPNS;
	private Button edit;
	private Button save;
	...
	public PegawaiDetailCtrl_DataPokok_PengangkatanPNS() {
		super();
	}
	...
	public void doAfterCompose(Component component) throws Exception {
		...
		if (this.arg.containsKey("ModuleMainController")) {
			...
			getPegawaiDetailCtrl_DataPokok().setPegawaiDetailCtrl_DataPokok_PengangkatanPNS(this);
	...
	public void onCreate$windowPegawaiDetail_DataPokok_PengangkatanPNS(Event event) throws Exception {
		this.buttonCtrl_Pegawai_DataPokok_PengangkatanPNS = new ButtonStatusCtrl(getUserWorkspace(), btnController_classPrefix, true, null, null, null, null, null, null, null, edit, null, save, cancel, null);
		...
		comboBox_pejabat.setSelectedIndex(pejabat.indexOf(gabunganDAO.getGabunganByKodeTabelAndKode(ConstantsText.KODETABEL_PEJABAT, getSelected().getKodePejPns())));
		
		ListModelList golru = new ListModelList(golonganRuangDAO.getAllGolonganRuang());
		comboBox_pejabat.setModel(golru);
		comboBox_pejabat.setItemRenderer(new GolonganRuangListModelItemRenderer());
		comboBox_pejabat.setSelectedIndex(golru.indexOf(golonganRuangDAO.getGolonganRuangByKode(getSelected().getKodeGolRuPns())));
				
		doFitSize(event);
		this.buttonCtrl_Pegawai_DataPokok_PengangkatanPNS.setInitEdit();
	}
	...
	private void doEdit() {
		doStoreInitValue();
		this.buttonCtrl_Pegawai_DataPokok_PengangkatanPNS.setBtnStatus_Edit();
	...
	private void doSave() throws InterruptedException {
		try {
			tpCpnsDAO.saveOrUpdate(getSelected());
			doStoreInitValue();
			// refresh the list
...
//		textBox_nip.setReadonly(b);
		...
		dateBox_tmtpns.setReadonly(b);
		comboBox_sumpah.setDisabled(b);
	}
	...
	public void doFitSize(Event event) {
		...
		this.borderLayout_PegawaiDataPokok_PengangkatanPNS.setHeight(String.valueOf(maxListBoxHeight) + "px");

		this.windowPegawaiDetail_DataPokok_PengangkatanPNS.invalidate();
	}
