...
public class PegawaiDetailCtrl_DataPokok_PengangkatanPNS extends GFCBaseCtrl implements Serializable {
	...
	private Combobox comboBox_sumpah;
	...
	public void onCreate$windowPegawaiDetail_DataPokok_Identitas(Event event) throws Exception {
		...
		if(gabunganDAO.getGabunganByKodeTabelAndKode("12", getSelected().getKodePejPns()) != null) {
			label_pejabat.setValue(gabunganDAO.getGabunganByKodeTabelAndKode("12", getSelected().getKodePejPns()).getNama());
		}
		if(golonganRuangDAO.getGolonganRuangByKode(getSelected().getKodeGolRuPns()) != null) {
			label_golru.setValue(golonganRuangDAO.getGolonganRuangByKode(getSelected().getKodeGolRuPns()).get(0).getNamaGolonganRuang());
		}
