 ...
 * 
 */
public class KedudukanPNSListModelItemRenderer implements ListitemRenderer, Serializable {

	private static final long serialVersionUID = 1L;
	private static final Logger logger = Logger.getLogger(KedudukanPNSListModelItemRenderer.class);
	...
	public void render(Listitem item, Object data) throws Exception {
		...
		final Gabungan kedudukanPNS = (Gabungan) data;
		Listcell lc;

		lc = new Listcell(kedudukanPNS.getKode());
		lc.setStyle("padding-left: 5px");
		lc.setParent(item);

		lc = new Listcell(kedudukanPNS.getNama());
		lc.setStyle("padding-left: 5px");
		lc.setParent(item);

		item.setAttribute("data", data);
		ComponentsCtrl.applyForward(item, "onDoubleClick=onDoubleClickedKedudukanPNSItem");
