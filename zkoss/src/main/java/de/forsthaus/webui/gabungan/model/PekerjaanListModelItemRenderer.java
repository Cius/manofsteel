 ...
 * 
 */
public class PekerjaanListModelItemRenderer implements ListitemRenderer, Serializable {

	private static final long serialVersionUID = 1L;
	private static final Logger logger = Logger.getLogger(PekerjaanListModelItemRenderer.class);
	...
	public void render(Listitem item, Object data) throws Exception {
		...
		final Gabungan pekerjaan = (Gabungan) data;
		Listcell lc;

		lc = new Listcell(pekerjaan.getKode());
		lc.setStyle("padding-left: 5px");
		lc.setParent(item);

		lc = new Listcell(pekerjaan.getNama());
		lc.setStyle("padding-left: 5px");
		lc.setParent(item);

		item.setAttribute("data", data);
		ComponentsCtrl.applyForward(item, "onDoubleClick=onDoubleClickedPekerjaanItem");
