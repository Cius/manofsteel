 ...
 * 
 */
public class JenisOrganisasiListModelItemRenderer implements ListitemRenderer, Serializable {

	private static final long serialVersionUID = 1L;
	private static final Logger logger = Logger.getLogger(JenisOrganisasiListModelItemRenderer.class);
	...
	public void render(Listitem item, Object data) throws Exception {
		...
		final Gabungan jenisOrganisasi = (Gabungan) data;
		Listcell lc;

		lc = new Listcell(jenisOrganisasi.getKode());
		lc.setStyle("padding-left: 5px");
		lc.setParent(item);

		lc = new Listcell(jenisOrganisasi.getNama());
		lc.setStyle("padding-left: 5px");
		lc.setParent(item);

		item.setAttribute("data", data);
		ComponentsCtrl.applyForward(item, "onDoubleClick=onDoubleClickedJenisOrganisasiItem");
