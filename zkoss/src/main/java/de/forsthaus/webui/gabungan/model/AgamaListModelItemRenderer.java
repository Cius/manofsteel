 ...
 * 
 */
public class AgamaListModelItemRenderer implements ListitemRenderer, Serializable {

	private static final long serialVersionUID = 1L;
	private static final Logger logger = Logger.getLogger(AgamaListModelItemRenderer.class);
	...
	public void render(Listitem item, Object data) throws Exception {
		...
		final Gabungan agama = (Gabungan) data;
		Listcell lc;

		lc = new Listcell(agama.getKode());
		lc.setStyle("padding-left: 5px");
		lc.setParent(item);

		lc = new Listcell(agama.getNama());
		lc.setStyle("padding-left: 5px");
		lc.setParent(item);

		item.setAttribute("data", data);
		ComponentsCtrl.applyForward(item, "onDoubleClick=onDoubleClickedAgamaItem");
